...
$NestedESXiHostnameToIPs = @{
    ...
    "esxi-04" = "192.168.1.104"
    "esxi-05" = "192.168.1.105"
    "esxi-06" = "192.168.1.106"
    "esxi-07" = "192.168.1.107"
    "esxi-08" = "192.168.1.108"
}
...
$VCSAIPAddress = "192.168.1.99"
$VCSAHostname = "vcsa"
$VCSAPrefix = "22"
$VCSASSODomainName = "bullies.local"
...
if($uploadNSXBundlePath -eq 1) {
    My-Logger "Connecting to new vCenter Server $VCSADisplayName ..."
    $viConnection = Connect-VIServer $VCSAIPAddress -User "administrator@bullies.local" -Password $VCSASSOPassword -WarningAction SilentlyContinue
...
if($generateVCFImportConvertCommand -eq 1) {
    ...
    My-Logger " "
    My-Logger "python3 vcf_brownfield.py convert --vcenter `'$($VCSAHostname + "." + $VMDomain)`' --sso-user `'administrator@$($VCSASSODomainName)`' --domain-name `'$($VCFManagementDomainName)`' --nsx-deployment-spec-path `'$('/home/vcf/' + $NsxSpecJsonFile)`' --auto-proceed --sso-password 'VMware123!' --vcenter-root-password `'$($VCSARootPassword)`' --local-admin-password `'$($SddcManagerLocalPassword)`' --backup-password `'$($SddcManagerBackupPassword)`' --accept-trust --suppress-warnings"
}
